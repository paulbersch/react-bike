    ...
    geocode(searchTerm) {
        ...
        geocodeCallback = function(results, status) {
            ...
            if (status == 'OK') {
                ...
                let box = distanceBox(center.lat, center.lng, this.state.distance);
                let bounds = new google.maps.LatLngBounds(new google.maps.LatLng(box[0], box[1]), new google.maps.LatLng(box[2], box[3]));
                this.props.dispatch(updateBounds(bounds.toJSON()));
                this.props.dispatch(updateCenter(center.toJSON()));
            };
        };
